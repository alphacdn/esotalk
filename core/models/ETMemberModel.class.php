<?php
  ...
  class ETMemberModel extends ETModel {
    ...
    public function validateUsername($username, $checkForDuplicate = true) {
      ...
      if ($checkForDuplicate and ET::SQL()->select("1")->from("member")->where("username=:username")->bind(":username",
                                                                                                           $username)->exec()->numRows()
      )
        return "nameTaken";
    }
    ...
    public function validateEmail($email, $checkForDuplicate = true) {
      ...
      if ($checkForDuplicate and ET::SQL()->select("1")->from("member")->where("email=:email")->bind(":email",
                                                                                                     $email)->exec()->numRows()
      )
        return "emailTaken";
    }
    ...
    public function setGroups($member, $account, $groups = array()) {
      ...
      ET::activityModel()->create("groupChange", $member, ET::$session->user,
                                  array("account" => $account, "groups" => $groupNames));
