...
<li><span><i class='fa fa-user'></i><?php printf(T("By %s"), "<a href='{$plugin["info"]["authorURL"]}'>{$plugin["info"]["author"]}</a>"); ?></span></li>
<li><span><?php printf(T("Version %s"), $plugin["info"]["version"]); ?></span></li>
<li class='sep'></li>
<li><a href='<?php echo URL("admin/plugins/uninstall/$k?token=".ET::$session->token); ?>'><i class='fa fa-trash-o'></i><?php echo T("Uninstall"); ?></a></li>
</ul>
...
<a href='<?php echo URL("admin/plugins/toggle/$k?token=".ET::$session->token); ?>' class='button toggle'><?php echo $plugin["loaded"] ? "<i class='fa fa-check'></i> " : "<i class='fa fa-square-o'></i> "; ?></a>
</div>
