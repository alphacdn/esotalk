  ...
  class ETUserController extends ETController {
    ...
    public function action_login() {
      ...
      $form->addSection("password",
                        T("Password") . " <small><a href='" . URL("user/forgot") . "' class='link-forgot' tabindex='-1'>" . T("Forgot?") . "</a></small>");
      $form->addField("password", "password", function ($form) {
        ...
        function ($form, $key, &$success) use ($controller) {
          ...
          if (ET::$session->login($form->getValue("username"), $form->getValue("password"),
      ...
      if (isset($form->errors["emailNotYetConfirmed"])) {
        $this->renderMessage("Error", sprintf(T("message.emailNotYetConfirmed"),
                                              URL("user/sendConfirmation/" . $form->getValue("username"))));
        return;
      }
    ...
    public function action_join() {
      ...
      $form->addField("password", "password", function ($form) {
        return $form->input("password",
                            "password") . "<br><small>" . sprintf(T("Choose a secure password of at least %s characters"),
                                                                  C("esoTalk.minPasswordLength")) . "</small>";
      },
        function ($form, $key, &$data) {
    ...
    public function sendConfirmationEmail($email, $username, $hash) {
      sendEmail($email,
                ...
                sprintf(T("email.header"), $username) . sprintf(T("email.confirmEmail.body"), C("esoTalk.forumTitle"),
                                                                URL("user/confirm/" . $hash, true))
      );
    }
    ...
    public function action_sendConfirmation($username = "") {
      ...
      if ($member) {
        $this->sendConfirmationEmail($member["email"], $member["username"],
                                     $member["memberId"] . $member["resetPassword"]);
        $this->renderMessage(T("Success!"), T("message.confirmEmail"));
      ...
      if ($form->validPostBack("submit")) {
        ...
        if (!$member)
          ...
          sendEmail($member["email"],
                    ...
                    sprintf(T("email.header"), $member["username"]) . sprintf(T("email.forgotPassword.body"),
                                                                              C("esoTalk.forumTitle"),
                                                                              URL("user/reset/" . $member["memberId"] . $hash,
                                                                                  true))
          );
