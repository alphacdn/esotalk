<?php
  ...
  class ETSession extends ETModel {
    ...
    public function __construct() {
      ...
      session_name("aphacdn_cross_session");
    ...
    public function dashLogin() {
      if (!$_SESSION['is_valid'] || !isset($_SESSION['account_email'])) return;
      $email = $_SESSION['account_email'];
  
      // Get the member with this username or email.
      // replaced: ->where("m.username=:username OR m.email=:email")
      $sql = ET::SQL()
               ->where("m.email=:email")
               ->bind(":email", $email);
      $member = reset(ET::memberModel()->getWithSQL($sql));
    ...
    public function setCookie($name, $value, $expire = 0) {
      return setcookie(C("esoTalk.cookie.name") . "_" . $name, $value, $expire,
                       C("esoTalk.cookie.path", getWebPath('')), C("esoTalk.cookie.domain"));
    }
     ...
     * @param string $type The type of page this is (search/conversation/etc - will be used in the "back to [type]"
     *                     text.)
