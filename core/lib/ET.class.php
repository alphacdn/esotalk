<?php
  ...
  class ET {
    ...
    public static function checkForUpdates() {
      ...
      if (isset($packages["esoTalk"]) and version_compare($packages["esoTalk"]["version"], ESOTALK_VERSION,
                                                          ">") == -1
     ...
     * We need to store these so we can re-define them if we switch languages using saveLanguageState() and
     * loadLanguage().
     * @var array
     */
    ...
    public static function loadLanguage($language = "") {
      ...
      // Set the locale.
      if (isset(ET::$languageInfo[self::$language]["locale"])) setlocale(LC_ALL,
                                                                         ET::$languageInfo[self::$language]["locale"]);
